...
const STATE_STYLES: Record<string, string> = {
  NEW: "bg-sky-900/40 text-sky-300",
  LEARNING: "bg-yellow-900/40 text-yellow-300",
  REVIEW: "bg-green-900/40 text-green-300",
  RELEARNING: "bg-red-900/40 text-red-300",
};
  ...
  return (
    <span
      className={cn(
        ...
        STATE_STYLES[state as string] ?? "bg-zinc-800 text-zinc-300",
      )}
    >
      {state as string}
    </span>
...
export default function CardItem({ card, onUpdated, onDeleted }: CardItemProps) {
  ...
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-900 transition-shadow hover:shadow-sm">
      {/* Collapsed row */}
      <button
        type="button"
        onClick={() => {
      ...
      >
        {/* Word */}
        <span className="min-w-0 shrink-0 text-xl font-bold text-zinc-50">
          {card.word}
        </span>
              ...
              <div className="mt-4 flex gap-2">
                ...
                {confirmDelete ? (
                  ...
                  <button
                    ...
                    className="rounded-md border border-red-800 px-3 py-1.5 text-sm font-medium text-red-400 transition-colors hover:bg-red-900/30"
                  >
                    Delete
                  </button>
                )}
